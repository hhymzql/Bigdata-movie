...
public class CastCrewDAO{
    ...
    private static Table castCrewTable = null;
    private final static String TABLE_NAME="CASTCREW";
    ...
    public boolean insertMovieCast(CastCrewTO castTO) {
        ...
        if (castTO != null) {
            ...
            /*Row row = castCrewTable.createRowFromJson(jsonTxt, true);
    ...
    public CastCrewTO getMovieCast(int movieId) {
        ...
        if (movieId > -1) {
            /*PrimaryKey key = this.getMovieCastKey(movieId);
            Row row = getTableAPI().get(key,null);
            
            String castJsonTxt = row.toJsonString(true);
     ...
     */
    /*private PrimaryKey getMovieCastKey(int movieId) {
        PrimaryKey key = null;

        if (movieId > -1) {
           key = castCrewTable.createPrimaryKey();
           key.put(JsonConstant.ID,movieId);
            key.put("mid",movieId);
        }
        return key;
    } //getMovieCastKey
